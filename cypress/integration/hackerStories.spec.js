describe('Hacker Stories', () => {
  ...
  context('Hitting the real API', () => {
    beforeEach(() => {
      // cy.intercept('GET', '**/search?query=React&page=0').as('getReact') //first version

      cy.intercept({ method: 'GET', pathname: '**/search', query: { query: initialTerm, page: '0' } })
    ...
    it('shows 20 stories, then the next 20 after clicking "More"', () => {
      ...
      }).as('getReactSecondPage')

      cy.get('.item').should('have.length', 20)

      cy.contains('More').click()

      cy.wait('@getReactSecondPage')
